...
Systemet har fått flere forbedringer med tanke på sikkerhet og datavalidering:

## Brukerregistrering
- Det er ikke lenger mulig å registrere to brukere med samme e-postadresse. Dette forhindrer forvirring og sikrer unike brukerkontoer.
- Alle obligatoriske felt merket med stjernetegn (*) må fylles ut ved registrering. Dette sikrer at nødvendige brukerdata samles inn.
- Passord må oppfylle visse krav til kompleksitet (minst 8 tegn, inkludert store bokstaver, små bokstaver, tall og spesialtegn) for å øke sikkerheten.
- E-postadresser valideres for korrekt format under registrering.
- Ved registrering av nye brukere blir passord hashet før lagring i databasen, noe som beskytter brukerdata mot uautorisert tilgang.
- Brukerroller (pilot, registerfører) tildeles ved registrering og lagres i databasen for å styre tilgang til ulike deler av applikasjonen.
- Ved innlogging sjekkes både e-post og passord mot lagrede verdier i databasen for å sikre at kun autoriserte brukere får tilgang.
- Session management implementert for å holde brukere innlogget på en sikker måte, med tidsbegrensning på inaktivitet.
- Bruk av HTTPS for all kommunikasjon mellom klient og server for å beskytte data under overføring.
- Bruk av anti-CSRF tokens i skjemaer for å forhindre Cross-Site Request Forgery angrep.
- Input validering på både klient- og serversiden for å forhindre SQL-injeksjon og XSS-angrep.

# Obstacle-registrering og rapportering
- Submitted data valideres for å sikre at alle obligatoriske felt er fylt ut og at dataene er i riktig format før de lagres i databasen.
- Opplastede bilder valideres nå:
  - Filtyper: Kun tillatte filtyper (.jpeg, .png) aksepteres.
  - Filstørrelse: Maksimal filstørrelse settes for å forhindre overbelastning av serveren og forhindrering av potensielle DoS-angrep.
  - Feilhåndtering: Hvis et bilde ikke oppfyller kravene, gis brukeren en klar feilmelding før innsending.
- Brukere kan kun redigere sine egne innmeldinger, basert på deres bruker-ID og rolle:
  - Registerførere kan se alle rapporter og se hvem som har endret status på dem.
  - Piloter kan ikke endre status på rapporter, men registerførere kan.
  - Piloter kan kun redigere rapporter som er i Draft-status, ikke de som er sendt inn.
- Statusendringer på rapporter logges med tidsstempel og bruker-ID for sporbarhet.
- Draft krever bare noen valideringer for innsending.


## Generell sikkherhet
- Mailhog brukes til å fange opp e-poster i utviklingsmiljøet, slik at ekte e-poster ikke sendes ut under testing.
- Miljøvariabler brukes til å lagre sensitive data som databasepassord, slik at disse ikke er hardkodet i kildekoden.
